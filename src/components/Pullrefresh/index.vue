...
<!--        ></Loading>-->
<!--        <div class="nut-pull-refresh-container-topbox-text">{{ getPullStatus }}</div>-->
<!--        <slot v-if="state.status == 'pulling'" name="pulling">-->
<!--        </slot>-->
<!--        <slot v-if="state.status == 'loosing'" name="loosing"></slot>-->
<!--        <slot v-if="state.status == 'loading'" name="loading"></slot>-->
<!--        <slot v-if="state.status == 'complete'" name="complete"></slot>-->
...
<!--const swipeEndTime = ref<number>(0)-->
<!--const swipeSpeedThreshold = 0.5 // 调整阈值以确保合理的滑动速度-->
...
<!--  const swipeDuration = swipeEndTime.value - swipeStartTime.value-->
<!--  const swipeSpeed = touch.deltaY.value / swipeDuration-->

<!--  if (state.isPullRefresh && isCanTouch() && touch.deltaY.value) {-->
<!--    if (swipeSpeed > 0 && swipeSpeed < swipeSpeedThreshold) { // 增加判断条件，确保滑动速度在合理范围内-->
<!--      if (state.status === 'loosing') {-->
<!--        setPullStatus(+props.headHeight, true)-->
<!--        emit('update:modelValue', true)-->
<!--        nextTick(() => emit('refresh'))-->
